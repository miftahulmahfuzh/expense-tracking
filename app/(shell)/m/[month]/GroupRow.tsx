 ...
 *
 * THE WHOLE ROW IS THE LINK — 414px wide, ≥56px tall (design R-41). No trailing chevron: a
 ...
 * photo evidence in the mono meta line as `⧉ 3`, and roadmap §5 lists the row as "title, item
...
export function GroupRow({ group }: { group: MonthGroupRow }) {
  return (
    <li className="border-b border-rule-2 last:border-b-0">
      <Link
        href={`/e/${group.id}`}
        className="flex min-h-row-lg press items-center gap-3 px-4 py-2.5"
      ...
      >
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-0.5 block font-mono tabular text-meta text-ink-3">
